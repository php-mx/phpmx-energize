...
const _app = {
  ...
  vue: {
    mountApp(component, elementId) {
      Vue.createApp(component).mount(elementId);
    },
    encapsulate(value) {
  ...
  lamp(toggle = true) {
    localStorage.lamp = localStorage.lamp ?? -1;
    if (toggle) localStorage.lamp *= -1;
    document.querySelectorAll(".useLamp").forEach((element) => element.classList.toggle("lamp", localStorage.lamp == 1));
  },
  alert(listAlert) {
    ...
    listAlert.forEach((item) => {
      ...
      let type = item[2] ?? "";
      let svg = {
        neutral: _app.ICON.neutral,
        success: _app.ICON.success,
        error: _app.ICON.error,
      }[type];
  ...
  formatDate(time, format = null) {
    if (time === true) time = new Date().getTime();
    ...
    if (!format) return date.getTime();

    let [formatedDate, formatedTime] = `${format} `.split(" ");

    if (formatedDate)
      formatedDate = formatedDate
        .split("/")
        .map((v) => {
          if (v == "d") return date.getDate().toString().padStart(2, "0");
          if (v == "m") return (date.getMonth() + 1).toString().padStart(2, "0");
          if (v == "y") return `${date.getFullYear()}`.substr(-2);
          if (v == "D") return date.getDate().toString().padStart(2, "0");
          if (v == "M") return (date.getMonth() + 1).toString().padStart(2, "0");
    ...
    if (formatedTime)
      formatedTime = formatedTime
        .split(":")
        .map((v) => {
          if (v == "h") return date.getHours().toString().padStart(2, "0");
          if (v == "i") return date.getMinutes().toString().padStart(2, "0");
          if (v == "s") return date.getSeconds().toString().padStart(2, "0");
          return v;
        })
        .join(":");

    return [formatedDate, formatedTime].join(" ").trim();
  },
  formatCoin(value) {
  ...
  favorite(ref) {
    ...
    let favoriteUrl = "/lobby/favorite/";
    _app.core
      .request(favoriteUrl + ref, "get", {}, { "Request-Fragment": true }, true)
      .then((resp) => {
        ...
        if (document.getElementById("FAVORITES")) _app.fragment(favoriteUrl, document.getElementById("FAVORITES"));
      })
      .catch(() => null);
  },
